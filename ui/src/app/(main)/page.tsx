...
import { fetchDirectory, addPinnedFolder, removePinnedFolderByPath } from "../../services/api"
import type { BrowseResponse, ViewMode, FileItem } from "../../types"
...
export default function DrivePage() {
    ...
    const { isAuth, loadingAuth, selectedDrive, pinnedFolders, refreshPinnedFolders } = useAppContext()
    ...
    useEffect(() => {
        if (isAuth && !loadingAuth && selectedDrive) {
            loadFiles(currentPath)
        }
    }, [isAuth, loadingAuth, currentPath, selectedDrive])
            ...
            <div className="px-6 py-4 flex items-center justify-between bg-white dark:bg-gray-900 z-10 sticky top-0 border-b border-gray-100 dark:border-gray-800 min-h-[73px]">
                <div className="flex flex-col gap-1">
                    <h1 className="text-2xl font-normal text-gray-800 dark:text-gray-100 tracking-tight">
                        {currentPath === "/" ? selectedDrive : currentPath.split("/").pop()}
                    </h1>
